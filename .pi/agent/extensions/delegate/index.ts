...
export default function delegateExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Use delegates for bounded, separable work: repetitive searches, mechanical migration sweeps, isolated implementation tasks, test/debug loops, narrow fact-finding, or reading a large body of context to answer one specific disposable question.",
      "Do not delegate first-pass project orientation, broad investigation, design synthesis, persona/prompt judgment, or context that the trunk session will need in order to supervise the work intelligently.",
      "Examples — do not delegate: investigating how a repo works; understanding Pi session lineage to design forum-native handoff; reviewing prompt/persona behavior; choosing feature architecture; reading manageable core docs for a system you are about to modify.",
      ...
      "Mixed cases depend on phase: if docs/logs/tests may teach concepts the trunk will use later, handle them directly; if they answer one narrow compatibility, citation, or red/green question, delegate.",
